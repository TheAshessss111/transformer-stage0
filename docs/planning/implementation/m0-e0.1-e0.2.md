...
`docs/planning/roadmap.md` puts gradcheck in E0.7 as a Python-side facility (F0.7.7). But E0.2 hand-writes ~15 VJPs,
...
This is A-01 from `docs/architecture/overview.md`, and it drives the API shape:
...
- Write `docs/architecture/engine.md`: the public API surface of `core/tensor`, the view/copy contract, and the
...
- [x] `docs/architecture/engine.md` exists and matches the code

### What changed during implementation

Five things the plan got wrong or could not have known. All are reflected in the
code and in `docs/architecture/overview.md`.
...
- `core/numerics/` (float.ts, hazards.ts) is listed in `docs/architecture/overview.md` but
...
T11 trace hook seam ────────────────────┤
                  ...
                  T17 softmax/CE                   T18 autograd
                        └───────────────┬───────────────┘
                                  T19 jacobian
                                        │
                                  T20 verify v3 + docs/architecture/engine.md    [E0.2]
```
